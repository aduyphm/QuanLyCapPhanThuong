...
public class HocSinhController extends EntityController<HocSinh>{
    ...
    public ArrayList<PhanQua> getAllGift(){
        ...
        ResultSet result = DatabaseConnection
                .executeQuery("Select * from phan_qua where loaiQua like '%Khuyến học%'");

        try {
            while (result.next()) {
                returnResult.add(new PhanQua(
                    result.getInt("ID"), 
    ...
    public Map<String, Integer> getAllStudentsPerType(){
        ...
        result = DatabaseConnection
                .executeQuery("SELECT COUNT(*) as Num FROM hoc_sinh WHERE hoc_sinh.thanhTichDacBiet IS NOT NULL AND hoc_sinh.thanhTichDacBiet != ''");
    ...
    public void giveGift(Date theGivingDay, int studyYear, ArrayList<PhanQua> phanQuas, 
        Integer[] index, int sumGift, int sumValue) throws SQLException{
        ResultSet result1 = DatabaseConnection.executeQuery("SELECT * FROM phan_qua_khuyen_hoc WHERE phan_qua_khuyen_hoc.date = '" + theGivingDay + "'");

        if(result1.next()){
            throw new SQLException();
        }

        DatabaseConnection.executeUpdate("INSERT INTO phan_qua_khuyen_hoc VALUES ('" 
        + theGivingDay + "', " 
        + phanQuas.get(index[0]) + ", "
        + phanQuas.get(index[1]) + ", "
        + phanQuas.get(index[2]) + ", "
        + sumGift + ", "
        + sumValue + ")");
        ResultSet result = DatabaseConnection
                .executeQuery("Select * from hoc_sinh where 1");
        ...
        for (int i = 0; i < arrayList.size(); i++) {
            ...
            if(arrayList.get(i).getThanhTichDacBiet() != null && !arrayList.get(i).getThanhTichDacBiet().isBlank()){
                DatabaseConnection
                    .executeUpdate("INSERT INTO `phat_qua_cuoi_nam` VALUES (null, "
