...
public class KhuyenHocTheoNhaController extends EntityController<KhuyenHocTheoNha> {
    ...
    private Date date = null;//= Date.valueOf("2020/12/02");
    ...
    public void getDataFromDatabase(int limit, int page) {
        ...
        if(date == null) {
            ResultSet result = DatabaseConnection
                .executeQuery("SELECT * FROM phat_qua_cuoi_nam INNER JOIN phan_qua ON phan_qua.ID = phat_qua_cuoi_nam.idPhanQua "
        ...
        + "tB.giaTri AS 'giaTri Trung bình', tB.moTa AS 'moTa Trung bình' "
        + "FROM phan_qua_khuyen_hoc "
        ...
        try {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                phanQua.put(curLoaiPhanQua, new PhanQua(
                    phanQuaResultSet.getInt(hocLuc[i]), 
                    "Khuyến học",
        ...
        //ArrayList<Pair<String, Date>> phanQuaKhuyenHoc = new ArrayList<>();
        
        try {
            metaData = result.getMetaData();
            
            while (result.next()) {
                //Pair<String, Date> curLoaiPhanQua = new Pair<String, Date>(result.getString("hocLuc"), phanQuaResultSet.getDate("date"));

                PhanQua cur = getPhanQua(result, phanQua);
                Pair<String, Date> curNhaKey = new Pair<String, Date>(result.getString("diaChi"), result.getDate("ngay"));
                if(!nha.containsKey(curNhaKey)){
                    KhuyenHocTheoNha temp = new KhuyenHocTheoNha(
            ...
            for (Map.Entry<Pair<String, Date>, KhuyenHocTheoNha> record : nha.entrySet()) {
                ...
                for (Map.Entry<PhanQua, Integer> num : temp.getMap().entrySet()) {
                    ...
                    sum += num.getValue() * num.getKey().getGiaTri();
                }
    ...
    public ArrayList<String> getNgayPhatQua(){
        ...
        ResultSet result = DatabaseConnection
                .executeQuery("SELECT * FROM phan_qua_khuyen_hoc WHERE 1");
        try {
            while (result.next()) {
